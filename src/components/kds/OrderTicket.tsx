...
import { Clock, AlertTriangle, Zap, Star, Check } from 'lucide-react';
...
export default function OrderTicket({ order, isSelected, onSelect, onMarkDone }: Props) {
  ...
  const groupedItems = useMemo(() => {
    const map = new Map<string, { menuItemId: string; quantity: number; notes: string[]; indices: number[]; allDone: boolean }>();
  ...
  if (isRushOrVip) {
    ...
    timerClass = pct > 0.75 ? 'bg-sla-green' : pct > 0.5 ? 'bg-sla-yellow' : 'bg-sla-red';
  } else {
    ...
    timerClass = ratio < 0.5 ? 'bg-sla-green' : ratio < 0.75 ? 'bg-sla-yellow' : 'bg-sla-red';
  }
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-1.5">
        ...
        <div className={`${timerClass} text-foreground px-2 py-0.5 rounded text-xs font-mono-data font-bold`}>
          ...
          return (
            <li
              ...
              className={`text-sm flex items-center justify-between gap-1 px-1.5 py-0.5 rounded transition-colors ${
                group.allDone ? 'bg-muted/50 text-muted-foreground line-through' : 'text-card-foreground'
              }`}
            >
              <span className="flex items-center gap-1.5">
                ...
                <span>{group.quantity}× {mi?.name || 'Unknown'}</span>
              </span>
              {group.notes.length > 0 && <AlertTriangle size={12} className="text-accent shrink-0" />}
            </li>
          );
        })}
      </ul>
